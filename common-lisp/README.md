# mask-pii (Common Lisp)

Version: 0.2.0

A lightweight, customizable Common Lisp library for masking Personally Identifiable Information (PII) such as **email addresses** and **phone numbers**.
...
Quicklisp (after the project is registered):

```lisp
(ql:quickload :mask-pii)
```

From a local checkout with ASDF:

```lisp
(asdf:load-system :mask-pii)
```
...
The `Masker` struct uses a builder-style API. By default, `new-masker` performs **no masking** (pass-through).
