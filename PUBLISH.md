...
| D | DUB | `mask-pii` | `BLOCKED` | — | 未割当 | D-02でサブディレクトリの直接登録不可を確認済み。D-03で直下recipeまたはsplit／mirrorを選定する。 |
...
- [ ] **D-03** リポジトリ直下のrecipeで `d/source` を公開する構成、またはD専用split／mirrorリポジトリについて承認を得る。
