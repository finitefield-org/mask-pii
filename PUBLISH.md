...
| --- | --- | --- | --- | --- | --- | --- |
| AWK | なし・Gitソース配布 | `awk/` | `N/A` | — | — | Gate: S0。Git導入手順を維持する。`awk-pkg` は任意のコミュニティ配布。 |
| Bash | なし・Git/bpkg配布 | `finitefield-org/mask-pii/bash` | `N/A` | — | — | Gate: S0。タグ付きGit導入手順を維持する。 |
| Bun | npm | 推奨: `@finitefield-org/mask-pii-bun` | `BLOCKED` | — | 未割当 | Gate: S0。スコープと名前を確保し、マニフェストを変更する。現在の非スコープ名は衝突し、JavaScript版とも重複。 |
| Carbon | なし・パッケージングは実験段階 | `carbon/` | `N/A` | — | — | Gate: S0。安定した公開レジストリが定義された時点で再調査する。 |
| Common Lisp | Quicklisp | システム名 `mask-pii` | `BLOCKED` | — | 未割当 | Gate: S0。モノレポ内ASDFの検出可否を確認後、Quicklispへ申請する。 |
| Crystal | Shards＋標準的な検索インデックス | shard名 `mask_pii` | `BLOCKED` | — | 未割当 | Gate: S0。サブディレクトリから導入できることを確認してからインデックスへ申請する。 |
| D | DUB | `mask-pii` | `BLOCKED` | — | 未割当 | Gate: S0。D-03でリポジトリ直下recipe方式を採用。D-04でroot recipeを実装・検証する。 |
| Deno | JSR | 推奨: `@finitefield/mask-pii` | `BLOCKED` | — | 未割当 | Gate: S0。JSRスコープを確保し、非スコープの `deno.json` 名を変更する。 |
| Elixir | Hex | `mask_pii` | `DONE` | `0.2.0` | Finite Field | Gate: S0。2026-07-17確認: https://hex.pm/packages/mask_pii |
| Fish | なし・FisherはGitから取得 | `finitefield-org/mask-pii` | `N/A` | — | — | Gate: S0。Git/Fisher導入手順を維持する。 |
| Go | Go Module Proxy／pkg.go.dev | `github.com/finitefield-org/mask-pii/go` | `PARTIAL` | `0.2.0` | Finite Field | Gate: S0。2026-07-17確認: https://pkg.go.dev/github.com/finitefield-org/mask-pii/go 。次回リリースでライセンスと文書を修正。 |
| Groovy | Maven Central | `org.finitefield:mask-pii` | `BLOCKED` | — | 未割当 | Gate: S0。Centralの名前空間、署名、認証情報、公開先リポジトリを設定する。 |
| Hare | 公式Hare Project Library | `maskpii` | `READY` | Git | 未割当 | Gate: S0。`HAREPATH` 手順確認後、hare-devメーリングリストへパッチを送る。 |
| Haskell | Hackage | `mask-pii` | `READY` | — | 未割当 | Gate: S0。source distributionを検証し、Hackageアカウントで公開する。 |
| JavaScript | npm | 推奨: `@finitefield-org/mask-pii` | `BLOCKED` | — | 未割当 | Gate: S0。非スコープ名 `mask-pii` は別所有者のため、スコープを確保しマニフェストと導入手順を更新する。 |
| Julia | General Registry | `MaskPII` | `PARTIAL` | `0.1.0` | Finite Field | Gate: S0。2026-07-17確認: https://juliahub.com/ui/Packages/General/MaskPII 。次回リリースで共通バージョンへ合わせる。 |
| Lua | LuaRocks | `mask-pii` | `BLOCKED` | — | 未割当 | Gate: S0。rockspecのモノレポ内ソースパスを修正・検証する。 |
| Nim | Nim package list／Nimble | `mask_pii` | `BLOCKED` | — | 未割当 | Gate: S0。package-list申請で `nim/` サブディレクトリを扱えるか確認する。 |
| Nushell | なし・Git／ファイル配布 | `nushell/` | `N/A` | — | — | Gate: S0。Git導入手順を維持し、公式レジストリ登場時に再調査する。 |
| OCaml | opam repository | `mask-pii` | `BLOCKED` | — | 未割当 | Gate: S0。不変URLとチェックサムを持つopam-repository用バージョンディレクトリを作る。 |
| Odin | サードパーティ用レジストリなし | `odin/mask_pii` | `N/A` | — | — | Gate: S0。collection-pathによる導入手順を維持する。 |
| Perl | CPAN／PAUSE | distribution `Mask-PII`／module `Mask::PII` | `READY` | — | 未割当 | Gate: S0。PAUSE名前空間権限を取得し、検証済みdistributionをアップロードする。 |
| PHP | Packagist | `finitefield-org/mask-pii` | `DONE` | `0.2.0` | Finite Field | Gate: S0。2026-07-17確認: https://packagist.org/packages/finitefield-org/mask-pii |
| Pony | なし・CorralはGit依存を解決 | `pony/mask_pii` | `N/A` | — | — | Gate: S0。タグ付きGit／Corral導入手順を維持する。 |
| PowerShell | PowerShell Gallery | `MaskPII` | `READY` | — | 未割当 | Gate: S0。マニフェスト検証後、APIキーで公開し、クリーン環境で導入確認する。 |
| Python | PyPI | `mask-pii` | `DONE` | `0.2.0` | Finite Field | Gate: S0。2026-07-17確認: https://pypi.org/project/mask-pii/ |
| R | CRAN | `maskpii` | `BLOCKED` | — | 未割当 | Gate: S0。`R CMD check --as-cran` の問題を解消し、CRAN用ライセンス形式を確認する。 |
| Racket | Racket Package Catalog | `mask-pii` | `BLOCKED` | — | 未割当 | Gate: S0。パッケージ内ライセンスを追加し、`racket/` を指すカタログソースを検証する。 |
| Red | なし・ソースファイル配布 | `red/mask-pii.red` | `N/A` | — | — | Gate: S0。Git／ファイル導入手順を維持する。 |
| Ruby | RubyGems | `mask-pii` | `DONE` | `0.2.0` | Finite Field | Gate: S0。2026-07-17確認: https://rubygems.org/gems/mask-pii |
| Rust | crates.io | `mask-pii` | `DONE` | `0.2.0` | Finite Field | Gate: S0。2026-07-17確認: https://crates.io/crates/mask-pii |
| Swift | Swift Package Index（SwiftPM配布はGit） | リポジトリURL | `READY` | Gitタグ `v0.2.0` | 未割当 | Gate: S0。Swift Package Indexへ申請し、ビルド互換性を確認する。 |
| Tcl | 権威あるレジストリなし | パッケージ名 `mask_pii` | `N/A` | — | — | Gate: S0。`pkgIndex.tcl`／Git導入を維持し、第三者カタログは任意とする。 |
| V | VPM | VPM互換のowner/package名 | `BLOCKED` | — | 未割当 | Gate: S0。モノレポのサブディレクトリ対応を確認し、非対応なら分離／ミラーする。 |
| Zig | なし・公式パッケージマネージャーはURL/hash方式 | `zig/` | `N/A` | — | — | Gate: S0。URL/hash依存を文書化し、存在しないレジストリへの申請は行わない。 |
| Zsh | なし・各種マネージャーはGitから取得 | `finitefield-org/mask-pii` | `N/A` | — | — | Gate: S0。タグ付きGit導入手順を維持する。 |

## 言語ページ進捗表

2026-07-21にfinitefield-siteの`codex/site-03`（`bb4c957cc8263b1101cc3a387fad98f755839546`）で全36言語の英語正本をコンテンツ契約へそろえた。英語entityのパスはfinitefield-siteの`web/content/entities/articles/oss/mask-pii/`からの相対パスである。登録先と公開状態は、重複管理を避けるため上のマスター進捗表を正本とする。

`現在のインストール欄`は非翻訳の`distributions.json`から全localeへ供給する`page.steps.installation.command`を示す。`完成`は配布状態、現在利用できる導入方法、読み込み、最小例、期待結果、実装・README・licenseリンク、公開情報または公開予定情報がそろった状態を示す。

不足キーの略号は、`status`（配布状態）、`install`（現在実行できる導入方法）、`load`（読み込み方法）、`example`（最小実行例）、`output`（期待結果）、`repo`（実装ディレクトリとREADMEへのリンク）、`license`（licenseリンク）、`release`（公開URL・版・確認日、または予定識別子・版・条件）を表す。opt-inの説明は全entityのhighlightに存在するため不足に含めない。

| 言語 | route | 英語entity | Web状態 | 現在のインストール欄 | README | 不足キー | 確認日 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| AWK | `/en/oss/mask-pii/awk/` | `awk/en.json` | `完成` | `awk -f awk/src/mask_pii.awk -f your_script.awk` | [あり](awk/README.md) | `なし` | 2026-07-21 |
| Bash | `/en/oss/mask-pii/bash/` | `bash/en.json` | `完成` | `bpkg install finitefield-org/mask-pii/bash` | [あり](bash/README.md) | `なし` | 2026-07-21 |
| Bun | `/en/oss/mask-pii/bun/` | `bun/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](bun/README.md) | `なし` | 2026-07-21 |
| Carbon | `/en/oss/mask-pii/carbon/` | `carbon/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](carbon/README.md) | `なし` | 2026-07-21 |
| Common Lisp | `/en/oss/mask-pii/common-lisp/` | `common-lisp/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](common-lisp/README.md) | `なし` | 2026-07-21 |
| Crystal | `/en/oss/mask-pii/crystal/` | `crystal/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](crystal/README.md) | `なし` | 2026-07-21 |
| D | `/en/oss/mask-pii/d/` | `d/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](d/README.md) | `なし` | 2026-07-21 |
| Deno | `/en/oss/mask-pii/deno/` | `deno/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](deno/README.md) | `なし` | 2026-07-21 |
| Elixir | `/en/oss/mask-pii/elixir/` | `elixir/en.json` | `完成` | `{:mask_pii, "~> 0.2.0"}` | [あり](elixir/README.md) | `なし` | 2026-07-21 |
| Fish | `/en/oss/mask-pii/fish/` | `fish/en.json` | `完成` | `fisher install finitefield-org/mask-pii` | [あり](fish/README.md) | `なし` | 2026-07-21 |
| Go | `/en/oss/mask-pii/go/` | `go/en.json` | `完成` | `go get github.com/finitefield-org/mask-pii/go@v0.2.0` | [あり](go/README.md) | `なし` | 2026-07-21 |
| Groovy | `/en/oss/mask-pii/groovy/` | `groovy/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](groovy/README.md) | `なし` | 2026-07-21 |
| Hare | `/en/oss/mask-pii/hare/` | `hare/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](hare/README.md) | `なし` | 2026-07-21 |
| Haskell | `/en/oss/mask-pii/haskell/` | `haskell/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](haskell/README.md) | `なし` | 2026-07-21 |
| JavaScript | `/en/oss/mask-pii/javascript/` | `javascript/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](javascript/README.md) | `なし` | 2026-07-21 |
| Julia | `/en/oss/mask-pii/julia/` | `julia/en.json` | `完成` | `import Pkg; Pkg.add(name="MaskPII", version="0.1.0")` | [あり](julia/README.md) | `なし` | 2026-07-21 |
| Lua | `/en/oss/mask-pii/lua/` | `lua/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](lua/README.md) | `なし` | 2026-07-21 |
| Nim | `/en/oss/mask-pii/nim/` | `nim/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](nim/README.md) | `なし` | 2026-07-21 |
| Nushell | `/en/oss/mask-pii/nushell/` | `nushell/en.json` | `完成` | `cp nushell/src/mask_pii.nu /path/to/your/nushell/module/path/` | [あり](nushell/README.md) | `なし` | 2026-07-21 |
| OCaml | `/en/oss/mask-pii/ocaml/` | `ocaml/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](ocaml/README.md) | `なし` | 2026-07-21 |
| Odin | `/en/oss/mask-pii/odin/` | `odin/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](odin/README.md) | `なし` | 2026-07-21 |
| Perl | `/en/oss/mask-pii/perl/` | `perl/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](perl/README.md) | `なし` | 2026-07-21 |
| PHP | `/en/oss/mask-pii/php/` | `php/en.json` | `完成` | `composer require finitefield-org/mask-pii` | [あり](php/README.md) | `なし` | 2026-07-21 |
| Pony | `/en/oss/mask-pii/pony/` | `pony/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](pony/README.md) | `なし` | 2026-07-21 |
| PowerShell | `/en/oss/mask-pii/powershell/` | `powershell/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](powershell/README.md) | `なし` | 2026-07-21 |
| Python | `/en/oss/mask-pii/python/` | `python/en.json` | `完成` | `pip install mask-pii` | [あり](python/README.md) | `なし` | 2026-07-21 |
| R | `/en/oss/mask-pii/r/` | `r/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](r/README.md) | `なし` | 2026-07-21 |
| Racket | `/en/oss/mask-pii/racket/` | `racket/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](racket/README.md) | `なし` | 2026-07-21 |
| Red | `/en/oss/mask-pii/red/` | `red/en.json` | `完成` | `cp red/mask-pii.red ./mask-pii.red` | [あり](red/README.md) | `なし` | 2026-07-21 |
| Ruby | `/en/oss/mask-pii/ruby/` | `ruby/en.json` | `完成` | `gem install mask-pii -v 0.2.0` | [あり](ruby/README.md) | `なし` | 2026-07-21 |
| Rust | `/en/oss/mask-pii/rust/` | `rust/en.json` | `完成` | `cargo add mask-pii@0.2.0` | [あり](rust/README.md) | `なし` | 2026-07-21 |
| Swift | `/en/oss/mask-pii/swift/` | `swift/en.json` | `完成` | `.package(url: "https://github.com/finitefield-org/mask-pii", exact: "0.2.0")` | [あり](swift/README.md) | `なし` | 2026-07-21 |
| Tcl | `/en/oss/mask-pii/tcl/` | `tcl/en.json` | `完成` | `lappend auto_path /path/to/mask-pii/tcl; package require mask_pii 0.2.0` | [あり](tcl/README.md) | `なし` | 2026-07-21 |
| V | `/en/oss/mask-pii/v/` | `v/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](v/README.md) | `なし` | 2026-07-21 |
| Zig | `/en/oss/mask-pii/zig/` | `zig/en.json` | `完成` | `zig fetch --save https://github.com/finitefield-org/mask-pii/archive/refs/tags/v0.2.0.tar.gz` | [あり](zig/README.md) | `なし` | 2026-07-21 |
| Zsh | `/en/oss/mask-pii/zsh/` | `zsh/en.json` | `完成` | `git clone --branch v0.2.0 --depth 1 https://github.com/finitefield-org/mask-pii.git` | [あり](zsh/README.md) | `なし` | 2026-07-21 |
