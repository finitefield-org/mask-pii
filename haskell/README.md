...
A lightweight, customizable Haskell library for masking Personally Identifiable Information (PII) such as **email addresses** and **phone numbers**.
...
```haskell
import MaskPII (Masker, newMasker, maskEmails, maskPhones, withMaskChar, process)
...
| `withMaskChar` | Sets the character used for masking (e.g., `'*'`, `'#'`, `'x'`). | `'*'` |
